...
def build_component_smooth_matrix(coeff_pinv, laplacian_matrix):
    """Project the voxel-space Laplacian into PCA component space."""
    coeff_pinv = np.asarray(coeff_pinv, dtype=np.float64)
    if sparse.issparse(laplacian_matrix):
        laplacian = laplacian_matrix.tocsr()
    else:
        laplacian = np.asarray(laplacian_matrix, dtype=np.float64)
    projected = coeff_pinv @ (laplacian @ coeff_pinv.T)
    return standardize_matrix(projected)

def build_behavior_correlation_matrices(beta_components, normalized_behaviors, ridge=1e-6):
    ...
    C_b = 0.5 * (C_b + C_b.T)
    C_d = 0.5 * (C_d + C_d.T)
    eye = np.eye(beta_components.shape[0], dtype=np.float64)
    C_b = C_b + ridge * eye
    C_d = C_d + ridge * eye
    return C_b, C_d
...
def prepare_data_func(projection_data, trial_indices, trial_length, run_boundary):
    ...
    C_corr_num, C_corr_den = build_behavior_correlation_matrices(beta_centered, normalized_behaviors)
...
def calcu_penalty_terms(run_data, alpha_task, alpha_bold, alpha_beta, alpha_smooth):
    ...
    if smooth_matrix is None:
        raise ValueError("C_smooth matrix unavailable in run_data.")
...
def _total_loss_from_penalty(weights, total_penalty): # Change this function/ also, plot each term separately 
    weights = np.where(np.isfinite(weights), weights, 0.0)
    return float(weights.T @ total_penalty @ weights)

def solve_soc_problem(run_data, alpha_task, alpha_bold, alpha_beta, alpha_smooth, solver_name, phro):
    beta_centered = run_data["beta_centered"]
    n_components = beta_centered.shape[0]
    penalty_matrices, total_penalty = calcu_penalty_terms(run_data, alpha_task, alpha_bold, alpha_beta, alpha_smooth)
    gamma_value = float(phro)

    corr_num = run_data["C_corr_num"]
    corr_den = run_data["C_corr_den"]
    A_mat = _symmetrize_matrix(gamma_value * total_penalty - corr_num)
    B_mat = _symmetrize_matrix(corr_den)
    ridge = 1e-8
    eye = np.eye(n_components, dtype=np.float64)
    A_mat = A_mat + ridge * eye
    B_mat = B_mat + ridge * eye
    ...
    def _objective_with_grad(weights):
        ...
        if denominator <= 0:
            print(1)
            return np.inf, np.ones_like(weights)
    ...
    initial_weights = np.full(n_components, 1.0 / n_components, dtype=np.float64)

    result = minimize(_objective, initial_weights, jac=_objective_grad,
    ...
    solution_weights = np.asarray(result.x, dtype=np.float64)
    solution_weights = np.maximum(solution_weights, 0.0)
    weight_sum = solution_weights.sum()
    if weight_sum <= 0:
        solution_weights = np.full(n_components, 1.0 / n_components, dtype=np.float64)
    else:
        solution_weights /= weight_sum

    denominator_value = float(solution_weights.T @ B_mat @ solution_weights)
    if denominator_value < min_den:
    ...
    total_loss = _total_loss_from_penalty(solution_weights, total_penalty)
    ...
    return {
        ...
        "branch": "fractional_qp",
        "total_loss": total_loss,
        "Y": y,
        "fractional_objective": fractional_objective,
        "numerator": numerator_value,
        "denominator": denominator_value,
        "penalty_contributions": contributions,
        "corr_ratio": correlation_ratio,
    }
...
def _compute_weighted_active_bold_projection(voxel_weights, data):
    ...
    projection = _compute_weighted_projection(voxel_weights, bold_matrix)
...
def _compute_weighted_projection(voxel_weights, measurements):
    ...
    weighted = measurements * weights[:, None]
...
def save_active_bold_correlation_map(correlations, active_coords, volume_shape, anat_img, file_prefix, result_prefix="active_bold_corr", map_title=None):
...
def _load_projection_series(series_path):
    ...
    try:
        ...
        print(f"Warning: failed to load stored gamma projections from {series_path}: {exc}", flush=True)
        return {}
    series = {}
    for key, values in data.items():
        try:
            ...
            continue
        series[gamma_key] = np.asarray(values, dtype=np.float64).ravel()
    return series
...
def plot_train_test_total_loss_box(train_values, test_values, title, ylabel, output_path):
    def _prepare(entries):
        ...
        fold_ids = np.array([int(idx) for idx, _ in sorted_entries], dtype=np.int64)
        raw_values = np.array([float(val) if np.isfinite(val) else np.nan for _, val in sorted_entries], dtype=np.float64)
        finite_mask = np.isfinite(raw_values)
...
def plot_metric_heatmap(combo_labels, metric_labels, metric_values, output_path, cmap="viridis"):
    if not combo_labels:
        print("Heatmap skipped: no alpha/gamma combinations available.", flush=True)
        return None
    ...
    ax.set_xlabel("Alpha / gamma combinations")
...
def save_projection_outputs(pca_weights, bold_pca_components, trial_length, file_prefix,
    ...
    # plot_projection_bold(y_trials, task_alpha, bold_alpha, beta_alpha, gamma_value, plot_path, series_label="PC space")
    ...
    if plot_trials:
        plot_path = f"y_projection_trials_{file_prefix}.png"
        plot_projection_bold(y_trials, task_alpha, bold_alpha, beta_alpha, gamma_value, plot_path,
...
# gamma_sweep = [0.2, 0.6, 0.8]
# gamma_sweep = [0, 0.2, 0.8]
# gamma_sweep = [0.1, 0.01, 0.001]
gamma_sweep = [0.001]
...
projection_data["C_smooth"] = build_component_smooth_matrix(projection_data["coeff_pinv"], laplacian_matrix)
...
def run_cross_run_experiment(alpha_settings, gamma_values, fold_splits, projection_data):
    ...
    def _format_loss_label(contributions, label):
        numeric_value = _format_numeric(contributions.get(label))
        return f"{numeric_value:.6f}" if numeric_value is not None else "nan"
    ...
    for fold_idx, split in enumerate(fold_splits, start=1):
        ...
        for alpha_setting in alpha_settings:
            ...
            for gamma_value in gamma_values:
                ...
                sweep_suffix = f"{alpha_prefix}_gamma{gamma_value:g}"
                ...
                try:
                    solution = solve_soc_problem(train_data, task_alpha, bold_alpha, beta_alpha, smooth_alpha, solver_name, gamma_value)
                except Exception as exc:
                    print(f"  Fold {fold_idx}: optimization failed for {combo_label} -> {exc}", flush=True)
                    continue

                numerator_value = _format_numeric(solution.get("numerator"))
                denominator_value = _format_numeric(solution.get("denominator"))
                ...
                loss_report = (
                    f"C_task: {_format_loss_label(penalty_contributions, 'task')}, "
                    f"C_bold: {_format_loss_label(penalty_contributions, 'bold')}, "
                    f"C_beta: {_format_loss_label(penalty_contributions, 'beta')}, "
                    f"C_smooth: {_format_loss_label(penalty_contributions, 'smooth')}"
                )
                ...
                if SAVE_PER_FOLD_VOXEL_MAPS:
                    save_active_bold_correlation_map(voxel_weights * 1000, train_data.get("active_coords"), anat_img.shape[:3], anat_img, file_prefix, result_prefix="voxel_weights")
    ...
    if fold_output_tracker:
        ...
        for metrics_key in sorted(fold_output_tracker.keys()):
            ...
            save_active_bold_correlation_map(weights_avg * 1000, active_coords, volume_shape, anat_img, avg_prefix,
                                             result_prefix="voxel_weights_mean", map_title=weights_title)

            bold_corr_stack = np.stack(fold_outputs["bold_corr"], axis=0)
            ...
            save_active_bold_correlation_map(bold_corr_avg, active_coords, volume_shape, anat_img, avg_prefix,
                                             result_prefix="active_bold_corr_mean", map_title=bold_title)

            beta_corr_stack = np.stack(fold_outputs["beta_corr"], axis=0)
            ...
            if bold_clean_full is not None:
                bold_projection_signal, _ = _compute_weighted_active_bold_projection(weights_avg, {"active_bold": bold_clean_full})
                _, bold_projection_trials = _reshape_projection(bold_projection_signal, trial_len)
                bold_plot_path = f"y_projection_bold_{avg_prefix}.png"
                plot_projection_bold(bold_projection_trials, task_alpha, bold_alpha, beta_alpha, gamma_value,
                                     bold_plot_path, series_label="Active BOLD space (weights avg)",
                                     trial_indices=np.arange(num_trials, dtype=np.int64))
    ...
    if aggregate_metrics:
        ...
        heatmap_metric_defs = [("train_corr", "Train corr"), ("test_corr", "Test corr"), ("train_p", "Train p-value"),
                               ("test_p", "Test p-value"), ("train_total_loss", "Train total loss"),
                               ("test_total_loss", "Test total loss")]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run a subset of alpha/gamma sweeps.")
    parser.add_argument("--alpha-idx", type=int, default=None,
                        help="0-based index into alpha_sweep; omit to iterate over all alphas.")
    parser.add_argument("--gamma-idx", type=int, default=None,
                        help="0-based index into gamma_sweep; omit to iterate over all gammas.")
    parser.add_argument("--combo-idx", type=int, default=None,
                        help="Single 0-based index over alpha×gamma combinations (overrides --alpha-idx/--gamma-idx).")
